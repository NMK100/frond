...
export class PorteurProjetComponent implements OnInit {
  ...
  user!:Users;
  constructor(private fb:FormBuilder, private route:Router,private data:DataService,private dataPorteur:PorteurProjetDataService) {
  ...
  valider(){
    if (this.conditionUtilisation.valid) {
      ...
      const porteurProjet = new PorteurProjet(this.user);
      console.log("Données du porteur de projet :", this.user.email);
      this.dataPorteur.addPorteur(this.user).subscribe({
        ...
        error: (error) => {
          console.error("Erreur lors de l'enregistrement du porteur de projet", error);
          // this.route.navigate(['inscription']);
        }
      });
